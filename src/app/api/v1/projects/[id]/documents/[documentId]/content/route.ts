...
import { projectDocumentError } from "@/workbench/project-document-http";
...
export const GET = withApiAuth(async (request: Request, { params }: { params: Promise<{ id: string; documentId: string }> }) => {
  try {
    ...
    const document = readProjectDocument(getAppDatabase(), id, documentId);
  ...
  } catch (error) { return projectDocumentError(request, error, "读取资料失败，请稍后重试。"); }
});
