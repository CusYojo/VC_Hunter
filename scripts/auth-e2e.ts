...
async function prepare(directory: string): Promise<NodeJS.ProcessEnv> {
  ...
  return { ...process.env, NODE_ENV: "production", PORT: "3107", VC_HUNTER_AUTH_ENABLED: "true", BETTER_AUTH_URL: origin,
...
async function main() {
  ...
  try {
    ...
    server = spawn(process.execPath, ["--import", join(process.cwd(), "tests/auth-e2e/model-fixture.mjs"), "node_modules/next/dist/bin/next", "start", "--hostname", "127.0.0.1", "--port", "3107"], { env, stdio: "ignore" });
    await waitUntilReady(server);
    const tests = spawn(process.execPath, ["node_modules/playwright/cli.js", "test", "--config", "playwright.auth.config.ts", ...process.argv.slice(2)], { env, stdio: ["ignore", "inherit", "ignore"] });
...
main().catch(() => { console.error("[auth-e2e] Isolated test setup failed. Check build, port 3107 and test configuration. No credentials printed."); process.exitCode = 1; });
